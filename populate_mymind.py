...
def populate():
    ...
    users = [{'username': 'Yousuf','age':21,'height':183,
                'weight':62,'sex':'Male','data':items[0:n],'steps':generate_steps()},
            {'username': 'Marianna','age':25,'height':173,
                        'weight':62,'sex':'Female','data':items[n:2*n],'steps':generate_steps()},
            {'username': 'Joseph','age':19,'height':183,
                        'weight':74,'sex':'Male','data':items[2*n:3*n],'steps':generate_steps()},
            {'username': 'Adam','age':55,'height':193,
                        'weight':90,'sex':'Male','data':items[3*n:4*n],'steps':generate_steps()},
            {'username': 'Rebecca','age':18,'height':153,
                        'weight':55,'sex':'Female','data':items[4*n:5*n],'steps':generate_steps()},
            {'username': 'Roberta','age':19,'height':167,
                        'weight':59,'sex':'Female','data':items[5*n:6*n],'steps':generate_steps()},
            {'username': 'Isabella','age':34,'height':160,
                        'weight':75,'sex':'Female','data':items[6*n:7*n],'steps':generate_steps()},
            {'username': 'Smith','age':40,'height':199,
                        'weight':130,'sex':'Male','data':items[7*n:8*n],'steps':generate_steps()},
            {'username': 'Lars','age':64,'height':140,
                        'weight':100,'sex':'Male','data':items[8*n:9*n],'steps':generate_steps()},
            {'username': 'Leopold','age':20,'height':153,
                        'weight':100,'sex':'Male','data':items[9*n:10*n],'steps':generate_steps()},]
    for user in users:
        try:
